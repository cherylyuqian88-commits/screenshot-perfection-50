...
export default function RecordsPage({ onNavigate }: Props) {
  ...
  const filteredRecords = doctorFilter === "all" ? records : records.filter(r => r.doctor === doctorFilter);
      ...
      <Card>
        <PanelTitle title="调参记录">
          <div className="flex items-center gap-2">
            ...
            <input className="border border-line rounded-[14px] bg-card px-3.5 py-2 text-foreground outline-none w-[280px] text-sm" placeholder="搜索用户姓名/电话号码" />
            <Btn>搜索</Btn>
          </div>
        </PanelTitle>
